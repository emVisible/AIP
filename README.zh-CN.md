...
内容提交 → 引擎快判 → 自动过 / 自动拦 / 转人工。面向中文后台的内容审核
（文章 / 评论 / 工单）。自动放错比转人工贵，所以 `auto_err` 必须为 0。

## 原则

1. **语义事件**：只传业务语义；超过 4KB 拒绝，大对象走引用（`body_ref` + `context.get`）。
2. **最小充分上下文**：每次决策只拿判题所需的字段。
3. **AI 无执行权**：引擎输出必须经过注册表校验与门控才能生效，不可绕过。
4. **门控**：置信度过线才自动执行，否则转人工。无可用模型引擎时 heuristic
   照拦不照放——兜底引擎只准自动拦截，放行需模型引擎（离线优先，执行边界 fail-closed）。
...
要求 `python3` + `node` + `pnpm`。主产品 stdlib 零依赖；有 `uv` 则用，无则回落
系统 python。端口被占时脚本直接 abort 并给出 pid（`./start.sh --force` 一并重起）。
...
| `laya:sidecar` | 自托管 Laya（$0），独立进程，默认 `multilingual` | 自动跳过 |
| `jev` | TypeSafe 云端 API（需 `TYPESAFE_API_KEY`） | 自动跳过 |
| `heuristic` | 关键词兜底 | 永远在线 |

顺序 env `ENGINE_ORDER` 可覆（默认成本优先）。
横评：`python3 -m eval.sweep samples.jsonl --engine {rules,laya:sidecar,jev,heuristic}`。
...
温度拟合几十条即有效；微调需 500+ 条（届时开 notebook，`MODEL_DIR` 切换
checkpoint，sweep A/B 回归）。`expected==review` 的行自动跳过；合成样本只验
链路，不提交拟合表。
...
| `LAYA_SIDECAR_URL` / `LAYA_SIDECAR_TIMEOUT` | 127.0.0.1:8685 / 15s | sidecar 地址与超时 |
...
`session.complete`(L0)。L3/L4 不存在——出现即转人工。
...
- [ ] 驳回结构化理由字段（可回流训练）
- [ ] 阈值预设（一键严格/均衡/宽松）
- [ ] submit 全异步（202 + 轮询；当前同步 + SSE 进度已够用，暂缓）
