...
---
*修订记录*
- 2026-08 v1.2 追加 §八 可逆资源纪律（D-4 修订：冻结字面、保留语义路线）
- 2026-08 v1.1 追加 §七 脚本编辑安全（api.py 中段误删事故固化）
- 2026-08 v1 初版（根基加固轮确立：分层/状态/路径/错误/演进/测试六纲）


## 八、可逆资源纪律（v1.2 新增，源自 Cordis 评估）

**脊柱 = 确定性引擎 + Registry 协议 + 会话审计；一切 surface 是可替换
肢体。任何肢体的框架不得殖民脊柱。**

- 新增长驻资源（连接/会话/订阅/池）必须经既有收口点登记：
  `bgjobs`（任务型）或 `NotificationHub`（流型）；禁止裸 try/finally 散装清理
- Cordis **字面引入冻结**；其三支柱语义（Context 树/effect 可逆副作用/
  Service 缝）在触发条件满足后经**原生迷你内核**落地：
    触发条件（任一）：① 第二常驻表面立项 ② Pack 热插需求
                      ③ 手写清理补丁再现 ≥2 次
- 前端同源 TS 场景：长驻表面 ≥3 时方可评估引入真 cordis 包
- 修订记录见 harness-fusion.md §0 D-4(v1.1)
