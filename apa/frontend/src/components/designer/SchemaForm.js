...
function VarPicker({ vars, onPick }) {
    ...
    if (!vars.length)
        return null;
    return (_jsxs("div", { className: "relative", children: [_jsx("button", { ref: btnRef, type: "button", onClick: toggle, className: "text-[10px] text-blue-500 hover:text-blue-700 px-1", title: "\u5F15\u7528\u524D\u5E8F\u6B65\u9AA4\u8F93\u51FA", children: "{}" }), open && pos && (_jsx("div", { style: { position: "fixed", top: pos.top, left: pos.left,
                    width: 256 }, className: "z-[60] bg-white border rounded-lg shadow-lg\n                        max-h-48 overflow-y-auto", children: vars.map(v => (_jsxs("div", { className: "px-3 py-1.5 hover:bg-blue-50 cursor-pointer text-xs", onClick: () => { onPick(v.path); setOpen(false); }, children: [_jsx("span", { className: "font-mono text-blue-600", children: `{{${v.path}}}` }), _jsx("span", { className: "text-slate-400 ml-2", children: v.preview }), _jsxs("span", { className: "text-slate-300 ml-1 text-[10px]", children: ["\u2190", v.stepId] })] }, v.path))) }))] }));
}
// ---- 类型化控件 ----
